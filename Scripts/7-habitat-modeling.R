...
# Input parameters
# Data filenames
landcoverName <- "landcover-with-roads.tif"
ageName <- "forest-age-code.tif"
densityName <- "forest-density.tif"
depositName <- "surficial-deposits-update.tif"
...
if(doGRASSSetup){
  ...
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, landcoverName), output="landcover", flags=c("overwrite", "o"))
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, ageName), output="forestAge", flags=c("overwrite", "o"))
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, densityName), output="forestDensity", flags=c("overwrite", "o"))
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, depositName), output="deposit", flags=c("overwrite", "o"))
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, drainageName), output="drainage", flags=c("overwrite", "o"))
  execGRASS("r.in.gdal", input=file.path(intermediatesDir, "Boundaries", studyAreaName), output="studyArea1", flags=c("overwrite", "o"))
}else{
  initGRASS(gisBase=gisBase, gisDbase=gisDbase, location=grassLocation, mapset='HabitatModelling', override=TRUE)
}

# Set the geographic region
execGRASS('g.region', n='465990', e='-499200', w='-729390', s='262950', res='30') 
...
speciesHabitatPatchParams <- read.csv(file.path(tabularDataDir, "speciesHabitatPatchParameters.csv"), header=TRUE)
speciesResistanceReclass <- read.csv(file.path(tabularDataDir, "speciesResistanceReclass.csv"), header=TRUE)
...
for(i in 1:length(speciesList)){
  ...
  rcl<-read.csv(paste0(tabularDataDir, "/speciesForestDensityReclass.csv"),header=TRUE)[,c('DensityCode',species)]
  ...
  rcl<-read.csv(paste0(tabularDataDir, "/speciesForestAgeReclass.csv"),header=TRUE)[,c('AgeCode',species)]
  ...
  rcl<-read.csv(paste0(tabularDataDir, "/speciesDrainageReclass.csv"),header=TRUE)[,c('DrainageCode',species)]
  ...
  rcl<-read.csv(paste0(tabularDataDir, "/speciesDepositReclass.csv"),header=TRUE)[,c('DepositCode',species)]
  ...
  rcl<-read.csv(paste0(tabularDataDir,"/speciesDistanceToMajorRoadReclass.csv"),header=TRUE)[,c('From','To',species)]
  ...
  rcl<-read.csv(paste0(tabularDataDir,"/speciesDistanceToMinorRoadReclass.csv"),header=TRUE)[,c('From','To',species)]
  write.table(paste0(rcl[,'From'],' thru ',rcl[,'To'],'=',rcl[,species]),'rule.txt',sep="",col.names=FALSE,quote=FALSE,row.names=FALSE)
  ...
  rcl<-read.csv(paste0(tabularDataDir,"/speciesDistanceToUrbanReclass.csv"),header=TRUE)[,c('From','To',species)]
  write.table(paste0(rcl[,'From'],' thru ',rcl[,'To'],'=',rcl[,species]),'rule.txt',sep="",col.names=FALSE,quote=FALSE,row.names=FALSE)
  ...
  rcl<-read.csv(paste0(tabularDataDir,"/speciesDistanceToForestReclass.csv"),header=TRUE)[,c('From','To',species)]
  write.table(paste0(rcl[,'From'],' thru ',rcl[,'To'],'=',rcl[,species]),'rule.txt',sep="",col.names=FALSE,quote=FALSE,row.names=FALSE)
  ...
  rcl<-read.csv(paste0(tabularDataDir, "/speciesDistanceToForestEdgeReclass.csv"), header=TRUE)[, c('From', 'To', species)]
  write.table(paste0(rcl[,'From'], ' thru ', rcl[,'To'], '=', rcl[,species]), 'rule.txt', sep="", col.names=FALSE, quote=FALSE, row.names=FALSE)
  ...
  if(species == 'RASY'){
    ...
    write.table(c('0 thru 2=NULL','3 thru 10000000=1'),'habitatWetlans.txt', sep="", col.names=FALSE, quote=FALSE, row.names=FALSE)
    execGRASS('r.reclass', input=paste0(species, '_habitatWetlandsCount'), output=paste0(species, '_habitatWetlandInside'), rules='habitatWetlans.txt', flags=c('overwrite'))
  ...
  rcl<-read.csv(paste0(tabularDataDir,"/speciesResistanceReclass.csv"),header=TRUE)[,c('LandcoverCode',species)]
  ...
  # execGRASS('g.region', n='403680', e='-391410', w='-691380', s='117930', res=paste0('30'))
  # 
  ...
  # suitabilityMap_01 <- raster(paste0(b03habitatDir, "/", species, "_habitatSuitability_", '30', 'm.tif')) %>%
  #   calc(., fun=rescaleR)
  # 
  # # Save maps as geotiffs
  # writeRaster(suitabilityMap_01, paste0(b03habitatDir, "/", species, "_habitatSuitability_01_", '30', 'm.tif'), overwrite=TRUE)   
  # 
  # # Add maps to mapset (to clip to focal region next)
  # execGRASS("r.in.gdal", input=paste0(b03habitatDir, "/", species, "_habitatSuitability_01_", '30', 'm.tif'), output=paste0(species, "_habitatSuitability_01"), flags=c("overwrite", "o"))
  # 
  #############################
  ...
  # # Clip maps to mask
  # execGRASS('r.mapcalc', expression=paste0(species, "_habitatSuitability_Focal_", '30', "m=", species, "_habitatSuitability * 1"), flags=c("overwrite"))
  # execGRASS('r.mapcalc', expression=paste0(species, "_habitatSuitability_01_Focal_", '30', "m=", species, "_habitatSuitability_01 * 1"), flags=c("overwrite"))
  # execGRASS('r.mapcalc', expression=paste0(species, "_habitatPatch_Focal_", '30', "m=", species, "_habitatLargeBinary * 1"), flags=c("overwrite"))
  # execGRASS('r.mapcalc', expression=paste0(species, "_resistance_Focal_", '30', "m=", species, "_resistance * 1"), flags=c("overwrite"))
  # # Save clipped maps as geotiffs
  # execGRASS('r.out.gdal', input=paste0(species, "_habitatSuitability_Focal_", '30', "m"), output=paste0(b03habitatDir, '/', species, '_habitatSuitability_Focal_', '30', 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
  # execGRASS('r.out.gdal', input=paste0(species, "_habitatSuitability_01_Focal_", '30', "m"), output=paste0(b03habitatDir, '/', species, '_habitatSuitability_01_Focal_', '30', 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
  # execGRASS('r.out.gdal', input=paste0(species, "_habitatPatch_Focal_", '30', "m"), output=paste0(b03habitatDir, '/', species, '_habitatPatch_Focal_', '30', 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
  # execGRASS('r.out.gdal', input=paste0(species, "_resistance_Focal_", '30', "m"), output=paste0(b03resistanceDir, '/', species, '_resistance_Focal_', '30', 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
  ...
  # Identify patches below minimum area threshold
#   minPatchSize<-as.numeric(subset(speciesHabitatPatchParams, Species==species, select=MinPatchSize))
#   execGRASS('r.reclass.area', input=paste0(species, "_habitatPatch_Focal_", '30', "m"), output=paste0(species, "_habitatSmall_Focal_", '30', "m"), value=minPatchSize-0.01, mode='lesser', flags=c('overwrite','d'))
#   # Identify patches above minimum area threshold
#   execGRASS('r.reclass.area', input=paste0(species, "_habitatPatch_Focal_", '30', "m"), output=paste0(species, "_habitatLarge_Focal_", '30', "m"), value=minPatchSize, mode='greater', flags=c('overwrite','d'))
#   
#   # Replace NAs with 0's to make a binary habitat layer
#   execGRASS('r.mapcalc', expression=paste0(species, '_habitatPatch_Focal_Filtered_', '30', 'm=if(isnull(', species, '_habitatLarge_Focal_', '30','m),0,',species, '_habitatLarge_Focal_', '30', 'm)'), flags=c('overwrite'))
#   
#   # Save maps as geotiffs
#   # Apply mask of focal region
#   execGRASS('r.mask', raster="studyArea0",flags=c("overwrite"))
#   execGRASS('r.out.gdal', input=paste0(species, "_habitatPatch_Focal_Filtered_", '30', "m"), output=paste0(b03habitatDir, '/', species, '_habitatPatch_Focal_Filtered_', '30', 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
#   # Remove mask from mapset
#   execGRASS('r.mask', raster="studyArea0",flags=c("r"))
#   
...
#   # Apply mask of focal region
#   execGRASS('r.mask', raster="studyArea0",flags=c("overwrite"))
#   execGRASS('r.out.gdal', input=paste0(species, "_habitatPatch_Focal_Filtered_", '30', "m"), output=paste0(b03habitatDir, '/', species, '_habitatPatch_Focal_Filtered_', newResolution, 'm.tif'), format='GTiff', createopt='COMPRESS=LZW', flags=c('overwrite'))
#   # Remove mask from mapset
#   execGRASS('r.mask', raster="studyArea0",flags=c("r"))
#   
#   # 480m
...
patchSummaryStatistics <- map_dfr(speciesList,
                                  ~{ 
                                    ...
                                    habitatPatchRaster <- raster(paste0(b03habitatDir, '/', .x, '_habitatPatch_Focal_Filtered_', '30', 'm.tif'))
                                    habitatSuitabilityRaster <- raster(paste0(b03habitatDir, '/', .x, "_habitatSuitability_Focal_", '30', 'm.tif'))
